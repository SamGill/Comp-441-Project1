...
void Bullet::update(float frameTime)
{
	...
	//spriteData.x = frameTime * bulletNS::SPEED;         // move ship along X 
	spriteData.y -= frameTime * bulletNS::SPEED;         // move ship along Y
	if (spriteData.y < 0)
		visible = false;

}

void Bullet::banditUpdate(float frameTime)
{
	Entity::update(frameTime);

	

	spriteData.y += frameTime * bulletNS::BANDIT_SPEED;
